...
#define JUMP_THRESHOLD 0.06 //maximum jump between velocities
#define JUMP_THRESHOLD_ATTITUDE 5 //maximum jump between attitudes in degree for every step
...
double pre_x = 0.0, pre_y = 0.0, pre_z = 0.0; //store previous velocity for velcities
double vx = 0.0, vy = 0.0, vz = 0.0; //velocities
double prev_vx = 0.0, prev_vy = 0.0, prev_vz = 0.0; //in order to filter out unreasonable jump;
...
// store previous attitude to get the differentiation
double pre_rho = 0.0, pre_theta = 0.0, pre_psi = 0.0;
double v_rho = 0.0, v_theta = 0.0, v_psi = 0.0;

//store previous Euler rate to filter out unreasonable numbers
double v_rho_pre = 0.0, v_theta_pre = 0.0, v_psi_pre = 0.0;

//attitude filters
double att_x[NUM_SAMPLES_MED], att_y[NUM_SAMPLES_MED], att_z[NUM_SAMPLES_MED];
double att_x_med[NUM_SAMPLES_AVG], att_y_med[NUM_SAMPLES_AVG], att_z_med[NUM_SAMPLES_AVG];
double att_x_avg[NUM_SAMPLES_AVG], att_y_avg[NUM_SAMPLES_AVG], att_z_avg[NUM_SAMPLES_AVG];

//Euler rate filters
double rho_vel[NUM_SAMPLES_MED], theta_vel[NUM_SAMPLES_MED], psi_vel[NUM_SAMPLES_MED];
double rho_vel_med[NUM_SAMPLES_AVG], theta_vel_med[NUM_SAMPLES_AVG], psi_vel_med[NUM_SAMPLES_AVG];
double rho_vel_avg[NUM_SAMPLES_AVG], theta_vel_avg[NUM_SAMPLES_AVG], psi_vel_avg[NUM_SAMPLES_AVG];
...
int main( int argc, char* argv[] )
{
  ...
  // Program options
  std::string HostName = "192.168.0.100:801";
  if( argc > 1 )
  {
    HostName = argv[1];
  }
      ...
      for( unsigned int SubjectIndex = 0 ; SubjectIndex < SubjectCount ; ++SubjectIndex )
      {
        ...
        for( unsigned int SegmentIndex = 0 ; SegmentIndex < SegmentCount ; ++SegmentIndex )
        {
          ...
          //                                                    << Adapt( _Output_GetSegmentGlobalRotationEulerXYZ.Occluded ) << std::endl;
            ...
            vicon_msg.position[0] = pos_x_avg[NUM_SAMPLES_AVG - 1];//median(pos_x, NUM_SAMPLES_MED);//_Output_GetSegmentGlobalTranslation.Translation[ 0 ];
            vicon_msg.position[1] = pos_y_avg[NUM_SAMPLES_AVG - 1];//median(pos_y, NUM_SAMPLES_MED);//_Output_GetSegmentGlobalTranslation.Translation[ 1 ];
            vicon_msg.position[2] = pos_z_avg[NUM_SAMPLES_AVG - 1];//median(pos_z, NUM_SAMPLES_MED);//_Output_GetSegmentGlobalTranslation.Translation[ 2 ];
            ...
            dt = (double)(vicon_msg.timestamp - time_tmp)/1e6; //dt in microseconds

            // differentiate the positions to get velocities
            vx = ((double)vicon_msg.position[0]/1e3 - pre_x)/(dt);
            vy = ((double)vicon_msg.position[1]/1e3 - pre_y)/(dt);
            vz = ((double)vicon_msg.position[2]/1e3 - pre_z)/(dt);

            //differentiate the attitudes to get Euler rate
            v_rho = ((double)vicon_msg.attitude[0] - pre_rho)/dt;
            v_theta = ((double)vicon_msg.attitude[1] - pre_theta)/dt;
            v_psi = ((double)vicon_msg.attitude[2] - pre_psi)/dt;

        ///////////////////////filter the velocity
        //collect data in to stack
            //filter out unreasonable jump
            if(abs(vx - prev_vx) <= JUMP_THRESHOLD) addArr(vel_x, vx, NUM_SAMPLES_MED);
            if(abs(vy - prev_vy) <= JUMP_THRESHOLD) addArr(vel_y, vy, NUM_SAMPLES_MED);
            if(abs(vx - prev_vz) <= JUMP_THRESHOLD) addArr(vel_z, vz, NUM_SAMPLES_MED);
            ...
            vy = vel_x_avg[NUM_SAMPLES_AVG-1];
            vz = vel_x_avg[NUM_SAMPLES_AVG-1];

            //filter the Euler rate
            if(abs(v_rho - v_rho_pre) <= rad2deg(JUMP_THRESHOLD_ATTITUDE)) addArr(rho_vel, v_rho, NUM_SAMPLES_MED);
            if(abs(v_theta - v_theta_pre) <= rad2deg(JUMP_THRESHOLD_ATTITUDE)) addArr(theta_vel, v_theta, NUM_SAMPLES_MED);
            if(abs(v_psi - v_psi_pre) <= rad2deg(JUMP_THRESHOLD_ATTITUDE)) addArr(psi_vel, v_psi, NUM_SAMPLES_MED);
            ...
            v_psi = theta_vel_avg[NUM_SAMPLES_AVG - 1];
            ...
            pre_x = (double)vicon_msg.position[0]/1e3;
            pre_y = (double)vicon_msg.position[1]/1e3;
            pre_z = (double)vicon_msg.position[2]/1e3;
